...
Examples: `feat(didwebvh-server): add configurable serviceAccount`, `fix(didwebvh-server): correct env var name`
...
- [ ] Values are annotated (`@param`) and README regenerates cleanly (if applicable)
...
When the automated Release PR is created (e.g., `release/didwebvh-server-vX.Y.Z`), add the `skip-release` label to defer publishing.
